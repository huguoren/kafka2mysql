...
public class KafkaToMySQLJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        KafkaSource<String> kafkaSource = KafkaSource.<String>builder().setBootstrapServers("10.150.9.37:9092,10.150.9.43:9092,10.150.9.45:9092") // 替换为你的Kafka地址
                .setTopics("KafkaToMySQLJob") // 替换为你的输入Topic
                .setGroupId("KafkaToMySQLJob-consumer-group").setStartingOffsets(
                        OffsetsInitializer.committedOffsets(OffsetResetStrategy.EARLIEST)).setValueOnlyDeserializer(
                        new SimpleStringSchema()) // 注意：SimpleStringSchema 可能需要你自行处理序列化
                .build();
        ...
        mergedStream.addSink(JdbcSink.sink(
                // SQL语句
                "INSERT INTO user_actions (user_key, action_count) VALUES (?, ?) ON DUPLICATE KEY UPDATE action_count = ?", (preparedStatement, tuple) -> {
                ...
                new JdbcConnectionOptions.JdbcConnectionOptionsBuilder().withUrl("jdbc:mysql://10.150.9.31:3306/db_test").withDriverName(
                        "com.mysql.cj.jdbc.Driver").withUsername("fktest").withPassword("fktest").build())).name("MySQL Sink");
